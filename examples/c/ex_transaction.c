...
transaction_example(void) {
    ...
    error_check(session->create(session, "blue:transaction", "key_format=S,value_format=S"));
    /*! [transaction example table create] */
    ...
    error_check(session->open_cursor(session, "blue:transaction", NULL, NULL, &cursor));
    /*! [transaction example cursor open] */

    /*! [transaction example cursor insert] */
    cursor->set_key(cursor, "key1");
    cursor->set_value(cursor, "value1");  // 16 characters
    error_check(cursor->insert(cursor));

    cursor->set_key(cursor, "key1");
    cursor->set_value(cursor, "value2");  // 16 characters
    error_check(cursor->insert(cursor));
    /*! [transaction example cursor insert] */
