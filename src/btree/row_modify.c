...
/*
 * __wt_upd_alloc_with_vid --
 *     Allocate a WT_UPDATE structure and associated value and fill it in with version id.
 */
static inline int
__wt_upd_alloc_with_vid(WT_SESSION_IMPL *session, const WT_ITEM *key, const WT_ITEM *value, u_int modify_type, WT_UPDATE **updp,
  size_t *sizep)
{
    WT_UPDATE *upd;
    ...
    *updp = NULL;
    ...
    WT_ASSERT(session,
      (value == NULL && (modify_type == WT_UPDATE_RESERVE || modify_type == WT_UPDATE_TOMBSTONE)) ||
        (value != NULL &&
          !(modify_type == WT_UPDATE_RESERVE || modify_type == WT_UPDATE_TOMBSTONE)));
    WT_ASSERT(session, key->vid != NULL && key->vid_size > 0);
...
/*
 * __wt_row_modify_with_vid --
 *     Row-store insert, update and delete with vid.
 */
int
__wt_row_modify_with_vid(WT_CURSOR_BTREE *cbt, const WT_ITEM *key, const WT_ITEM *value, WT_UPDATE *upd_arg,
  u_int modify_type, bool exclusive, bool restore)
{
    WT_DECL_RET;
    WT_INSERT *ins;
    ...
    if (cbt->compare == 0) {
        ...
        if (upd_arg == NULL) {
            ...
            /* Allocate a WT_UPDATE structure and transaction ID. */
            WT_ERR(__wt_upd_alloc_with_vid(session, key, value, modify_type, &upd, &upd_size));
            upd->prev_durable_ts = prev_upd_ts;
            WT_ERR(__wt_txn_modify(session, upd));
            added_to_txn = true;
